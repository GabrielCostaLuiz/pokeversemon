  ...
  return (
    <div className="w-full h-full flex flex-col absolute overflow-hidden z-40">
      <Image
        ...
        alt="pokedex"
        sizes="100vw"
        className={`absolute overflow-hidden w-screen h-auto top-0 z-30 ${
          showPokeball ? "imgPokeball" : ""
        }`}
      />
      <div
        className={`bg-white flex-1 absolute z-20 overflow-hidden  w-screen h-[50vh] bottom-0 ${
          showPokeball ? "imgPokeballWhite" : ""
        }`}
      ></div>
    </div>
  );
